...
export default function ReasonNotEnoughJobs({ onAction, state, setState }: Props) {
  const [touched, setTouched] = useState(false);
  const value: string = state?.irrelevant_jobs_feedback ?? '';

  const setValue = (v: string) =>
    setState((s: any) => ({ ...s, irrelevant_jobs_feedback: v }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:order-1 lg:col-span-6">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-sm md:text-base text-gray-900">Not enough relevant jobs</span>
          </div>

          {/* Follow-up question */}
          <label className="mt-4 block text-sm md:text-base text-gray-900">
            In which way can we make the jobs more relevant?<sup className="text-gray-400">*</sup>
          </label>

          {/* Optional red helper when invalid */}
          {!isValid && touched && (
            <p className="mt-2 text-sm text-[#D32F2F]">
              Please enter at least {MIN} characters so we can understand your feedback
