...
export default function ReasonNotToMove({ onAction, state, setState }: Props) {
  const [touched, setTouched] = useState(false);
  const value: string = state?.not_to_move_feedback ?? '';

  const setValue = (v: string) =>
    setState((s: any) => ({ ...s, not_to_move_feedback: v }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:order-1 lg:col-span-6">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-sm md:text-base text-gray-900">Decided not to move</span>
          </div>

          {/* Follow-up question */}
          <label className="mt-4 block text-sm md:text-base text-gray-900">
            What changed for you to decide to not move?<sup className="text-gray-400">*</sup>
          </label>
